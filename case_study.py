...
initial_conditions = {
    ...
    "ic_6": np.array([5.,         7.2031784,  2.,         0.59320568]),
    "ic_7": np.array([1.89586903, 3.52321925, 0.,         2.42920099]),
    "ic_8": np.array([4.08484482e+00, 8.00000000e+00, 7.70965076e-04, 2.17497024e+00]),
    "ic_9": np.array([3.78119155e+00, 3.10674918e+00, 7.15564696e-04, 2.42282035e+00]),
    "ic_10": np.array([3.2377827,  6.80745908, 0.48906331, 2.42922345]),
    
    "ic_11": np.array([4.99999937, 3.,         0.72162555, 2.43864347]),
...
def save_matrix_as_csv(matrix, filename):
    ...
    filepath = os.path.join("Physics-Informed_ADoK/physics_informed_SR/exp_data_1", filename + ".csv")
...
#             try:
#                 if j == 0:
#                     file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_T_" + str(i + 1) + ".csv") 
#                 if j == 1:
#                     file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_H_" + str(i + 1) + ".csv")
#                 if j == 2:
#                     file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_B_" + str(i + 1) + ".csv")
#                 if j == 3:
#                     file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_M_" + str(i + 1) + ".csv")
...
#         try:
#             if i == 0:
#                 file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_rate_T_" + str(itera + 1) + ".csv")
#                 Y = SR_derivatives_T
#                 num = 200
#             if i == 1:
#                 file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_rate_H_" + str(itera + 1) + ".csv")
#                 Y = SR_derivatives_H
#                 num = 200
#             if i == 2:
#                 file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_rate_B_" + str(itera + 1) + ".csv")
#                 Y = SR_derivatives_B
#                 num = 200
#             if i == 3:
#                 file_name = str("Physics-Informed ADoK/sr_data/hall_of_fame_rate_M_" + str(itera + 1) + ".csv")
#                 Y = SR_derivatives_M
#                 num = 200
...
for i in range(num_species):
    if i == 0:
        path = "Physics-Informed ADoK/sr_data/hall_of_fame_rate_T_" + str(itera + 1) + ".csv"
        name_models = "T_models"
        name_params = "T_params"
    
    if i == 1:
        path = "Physics-Informed ADoK/sr_data/hall_of_fame_rate_H_" + str(itera + 1) + ".csv"
        name_models = "H_models"
        name_params = "H_params"
    
    if i == 2:
        path = "Physics-Informed ADoK/sr_data/hall_of_fame_rate_B_" + str(itera + 1) + ".csv"
        name_models = "B_models"
        name_params = "B_params"
    
    if i == 3:
        path = "Physics-Informed ADoK/sr_data/hall_of_fame_rate_M_" + str(itera + 1) + ".csv"
