...
for i in range(num_exp):
    ...
    for j in range(num_species):
        if j == 0:
            file_name = str("Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_T" + str(i + 1) + ".csv")
            name = "T_"
        if j == 1:
            file_name = str("Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_H" + str(i + 1) + ".csv")
            name = "H_"
        if j == 2:
            file_name = str("Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_B" + str(i + 1) + ".csv")
            name = "B_"
        if j == 3:
            file_name = str("Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_M" + str(i + 1) + ".csv")
            name = "M_"
...
def save_matrix_as_csv(matrix, filename):
    ...
    filepath = os.path.join("Physics-Informed_ADoK/physics_informed_SR/const_data_1", filename + ".csv")
    df.to_csv(filepath, index = False, header = False)

size = len(SR_derivatives_T)
save_matrix_as_csv(sr_data[:, 0:3].T, 'conc_data_for_rate_models')
save_matrix_as_csv(np.reshape(SR_derivatives_T, (1, size)), 'rate_data_T')
save_matrix_as_csv(np.reshape(SR_derivatives_H, (1, size)), 'rate_data_H')
save_matrix_as_csv(np.reshape(SR_derivatives_B, (1, size)), 'rate_data_B')
save_matrix_as_csv(np.reshape(SR_derivatives_M, (1, size)), 'rate_data_M')
...
for i in range(num_species):
    if i == 0:
        path = "Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_rate_T" + str(num_exp) + ".csv"
        name_models = "T_models"
        name_params = "T_params"
    
    if i == 1:
        path = "Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_rate_H" + str(num_exp) + ".csv"
        name_models = "H_models"
        name_params = "H_params"
    
    if i == 2:
        path = "Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_rate_B" + str(num_exp) + ".csv"
        name_models = "B_models"
        name_params = "B_params"
    
    if i == 3:
        path = "Physics-Informed_ADoK/physics_informed_SR/hof_files_1/hall_of_fame_rate_M" + str(num_exp) + ".csv"
