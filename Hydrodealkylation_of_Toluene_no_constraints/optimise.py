...
def change_numbers(string):
    ...
    string = string.replace('T', 'z[0]')
    string = string.replace('H', 'z[1]')
    string = string.replace('B', 'z[2]')
    string = string.replace('M', 'z[3]')

    return '(' + string + ')'
    
a = change_numbers('-0.28683272*H*T*(T + 0.851753)/(2.80980865989773*B + T*(T + 0.851753) + 0.491886424611766)')
b = change_numbers('(2.56 * H * T) / (1.54 + 3.56 * B)')
print(b)
...
def competition(k, z0):
    ...
    def nest(t, z):
        ...
        dHdt = (-1) * ((k_1 * z[1] * z[0]) / (k_2 + k_3 * z[2]))
        dBdt = ((k_1 * z[1] * z[0]) / (k_2 + k_3 * z[2]))
        dMdt = ((k_1 * z[1] * z[0]) / (k_2 + k_3 * z[2]))
